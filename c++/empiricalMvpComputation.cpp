...
struct HyperLogLogLogConfig {
  ...
  std::string getLabel() const { return "HyperLogLogLog"; }
};
...
private:
  ...
  uint64_t minimumInMemorySizeInBytes = std::numeric_limits<uint64_t>::max();
  uint64_t maximumInMemorySizeInBytes = std::numeric_limits<uint64_t>::min();
  uint64_t sumSerializationSizeInBytes = 0;
  uint64_t minimumSerializationSizeInBytes =
      std::numeric_limits<uint64_t>::max();
  uint64_t maximumSerializationSizeInBytes =
...
public:
  Statistics(uint64_t trueDistinctCount)
      : trueDistinctCount(trueDistinctCount) {}

  void add(uint64_t inMemorySizeInBytes, uint64_t serializedSizeInBytes,
           double distinctCountEstimate) {
    count += 1;
    minimumInMemorySizeInBytes =
        std::min(minimumInMemorySizeInBytes, inMemorySizeInBytes);
    maximumInMemorySizeInBytes =
        std::max(maximumInMemorySizeInBytes, inMemorySizeInBytes);
    sumInMemorySizeInBytes += inMemorySizeInBytes;
    minimumSerializationSizeInBytes =
        std::min(minimumSerializationSizeInBytes, serializedSizeInBytes);
    maximumSerializationSizeInBytes =
        std::max(maximumSerializationSizeInBytes, serializedSizeInBytes);
    sumSerializationSizeInBytes += serializedSizeInBytes;
    double distinctCountEstimationError =
        distinctCountEstimate - trueDistinctCount;
  ...
  uint64_t getTrueDistinctCount() const { return trueDistinctCount; }
  ...
  double getEstimatedInMemoryMVP() const {
    return getAverageInMemorySizeInBytes() * 8. *
           sumDistinctCountEstimationErrorSquared /
           (static_cast<double>(count) * trueDistinctCount * trueDistinctCount);
  }
...
template <typename T> void test(const T &config = T()) {
  ...
  for (uint64_t i = 0; i < num_cycles; ++i) {
    ...
    while (true) {
      if (distinct_count == distinct_counts[distinct_counts_idx]) {
        data[distinct_counts_idx].add(config.getInMemorySizeInBytes(sketch),
  ...
  std::ofstream o("results/comparison-empirical-mvp/" + config.getLabel() +
                  ".csv");

  o << "p = " << static_cast<uint64_t>(p)
    << "; number of cycles = " << num_cycles
    << "; data structure = " << config.getLabel() << std::endl;
