...
public class EmpiricalMVPComputation {
    ...
    protected abstract static class ZetaSketchConfig implements Config<HyperLogLogPlusPlus<Long>> {
        ...
        protected abstract HyperLogLogPlusPlus.Builder contribute(int p, HyperLogLogPlusPlus.Builder builder);
    ...
    private static final class Statistics {
        ...
        public void add(long inMemorySizeInBytes, long serializedSizeInBytes, double distinctCountEstimate) {
            count += 1;
            ...
            minimumSerializationSizeInBytes = Math.min(minimumSerializationSizeInBytes, serializedSizeInBytes);
            maximumSerializationSizeInBytes = Math.max(maximumSerializationSizeInBytes, serializedSizeInBytes);
            ...
            sumDistinctCountEstimationErrorSquared += distinctCountEstimationError * distinctCountEstimationError;
        }
        ...
        {
            return getAverageInMemorySizeInBytes() * 8. * sumDistinctCountEstimationErrorSquared / ((double)count * trueDistinctCount * trueDistinctCount);
        }

        double getEstimatedSerializationMVP()
        {
            return getAverageSerializationSizeInBytes() * 8. * sumDistinctCountEstimationErrorSquared / ((double)count * trueDistinctCount * trueDistinctCount);
        }
    }
    ...
    private static <T> void test(Config<T> config) {
        int p = 12;
        int numCycles = 10000;
        List<Statistics> statistics = LongStream.of(getDistinctCounts(1000000, 0.05)).mapToObj(Statistics::new).toList();
        ...
        for (int i = 0; i < numCycles; ++i)
        {
            ...
            while (true)
            {
                ...
                {
                    statistics.get(distinctCountsIdx).add(config.getInMemorySizeInBytes(sketch),
                            config.getSerializedSizeInBytes(sketch),
                            config.getEstimate(sketch, p));
                    distinctCountsIdx += 1;
                    if (distinctCountsIdx == statistics.size())
                        break;
                }
        ...
        try (FileWriter o = new FileWriter("../results/comparison-empirical-mvp/" + config.getLabel() + ".csv")) {

            o.write("p = " + p + "; number of cycles = " + numCycles + "; data structure = " + config.getLabel() + '\n');
